...
- **正規表現リテラルの追跡(2026-07 強化 / must-fix)**: `str.split(/\//)`・`/^\/api\//`・`/[/]/` のように `\/` 直後に `/` が続く形を `//` 行コメントと誤認し、その行の残り(`markDone()` 等の正当なコード)を丸ごと削っていた不具合を修正。学習者が提出する任意コードに対し `ignoreComments:true` の source check が実行されるため、正しいコードが不合格になる実害があった。標準的な簡易ヒューリスティック(直前の意味トークンが 識別子/数値/文字列/`)`/`]`/`}` なら `/` は除算、さもなくば正規表現の開始。文字クラス `[...]` と `\` エスケープを尊重)で判定し、迷うケースは「削らない」側に倒す(誤ってコードを消すより、コメントを消し残す方が安全)
...
- 既知の制限(いずれも「コメントを消し残す」安全側の失敗で、コードは誤って削らない): (1) 曖昧な `}`(ブロック終端か式終端か不定)は除算側に倒すため、ブロック直後の正規表現(`if(x){} /re/.test(s)`)は認識されない — 教材コードでは稀。(2) インライン `<script>` / `<style>` 内の JS/CSS コメントは除去対象外(HTML コメントのみ)。誤除去で solution が落ちる場合はステージ2が検出する
